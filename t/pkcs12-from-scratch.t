...
my $pkcs12_result = `openssl pkcs12 -export -out certs/scratch-pkcs12.p12 -inkey certs/test-key.pem -in certs/test-cert.pem -certfile certs/test-ca.pem -passout pass:testing`;
...
SKIP: {
    ...
    if (($major eq "3.1" and $minor <= 2) or ($major eq "3.0" and $minor <= 10)) {
        skip("OpenSSL 3.0.0 to 3.1.2 cannot change pkcs12 passwords", 3);
    } else {
